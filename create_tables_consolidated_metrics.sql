-- -- Dropping Database to start over:
DROP DATABASE IF EXISTS clickstack_consolidated;

-- -- -- Creating the Database to host the consolidated system tables:
CREATE DATABASE IF NOT EXISTS clickstack_consolidated;

-- -- Creating the consolidated query_log table:
CREATE TABLE IF NOT EXISTS clickstack_consolidated.query_log (
    ...
    `transaction_id` Tuple(
        UInt64,
        UInt64,
        UUID) COMMENT 'The identifier of the transaction in scope of which this query was executed.',
...
SETTINGS ttl_only_drop_parts = 1, merge_selecting_sleep_ms = '50000', index_granularity = 8192
...
-- -- -- Creating the consolidated asynchronous_metric_log table:
CREATE TABLE IF NOT EXISTS clickstack_consolidated.asynchronous_metric_log (
...
ORDER BY
    (metric, event_date, event_time) TTL event_time + toIntervalDay(30) SETTINGS ttl_only_drop_parts = 1,
    index_granularity = 8192 COMMENT 'Contains the historical values for system.asynchronous_metrics, once per time interval (one second by default).\n\nIt is safe to truncate or drop this table at any time.';


CREATE OR REPLACE TABLE clickstack_consolidated.metric_log
(
    `hostname` LowCardinality(String) COMMENT 'Hostname of the server executing the query.',
...
CREATE TABLE IF NOT EXISTS clickstack_consolidated.parts
(
    `partition` String,
    `name` String,
    `uuid` UUID,
    `part_type` String,
    `part_storage_type` String,
    `active` UInt8,
    `marks` UInt64,
    `rows` UInt64,
    ...
    `index_granularity_bytes_in_memory` UInt64,
    `index_granularity_bytes_in_memory_allocated` UInt64,
    `is_frozen` UInt8,
    `database` String,
    `table` String,
    `engine` String,
